...
const (
	// querySaveEvent inserts an event with multi-tenant + principal idempotency.
	// Uses composite key (tenant_id, principal_id, id) to prevent duplicate events.
	...
	querySaveEvent = `
		INSERT INTO events (
			id, tenant_id, principal_id, type, schema_version,
			occurred_at, ingested_at, metadata, data
		)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		ON CONFLICT (tenant_id, principal_id, id) DO NOTHING
	...
	// Note: Fetches events for ALL tenants and principals (sweeper processes globally).
